...
int main()
{
	...
	// Contacts
	std::ofstream con_file("output/contacts_with_time.txt");
	...
	// Contacts
	std::vector<std::vector<int>> contacts = abm.get_all_contacts();
	for (const auto& step : contacts) {
		std::copy(step.cbegin(), step.cend(), std::ostream_iterator<int>(con_file, " "));
		con_file << '\n';
	}
