...
bool contributions_main_test()
{
	...
	std::string cp_file("test_data/carpools.txt");
	std::string pt_file("test_data/public_transit.txt");
	std::string ls_file("test_data/public_leisure.txt");
	...
	// File with time dependent testing parameters
	...
	abm.set_outside_workplace_transmission();
	abm.set_outside_leisure_transmission();
	...
	// 3) Contributions with testing and quarantining
	for (auto& agent : nc_agents){
...
bool check_all_places(ABM& abm, const std::vector<Agent>& agents)
{
	...
	if (!check_regular_contributions<Transit>(carpools, agents, "carpool", infection_parameters)){
		std::cerr << "Wrong contribution in the carpool computation" << std::endl;
		return false;
	}
	if (!check_regular_contributions<Transit>(public_transit, agents, "public transit", infection_parameters)){
		std::cerr << "Wrong contribution in the public transit computation" << std::endl;
		return false;
	}
	if (!check_regular_contributions<Leisure>(leisure_locations, agents, "leisure location", infection_parameters)){
		std::cerr << "Wrong contribution in the leisure locations computation" << std::endl;
		return false;
	}
	return true;
}
