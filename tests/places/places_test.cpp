...
bool place_test()
{
	...
	if (place.outside_town() == true) {
		std::cerr << "All places except for some workplaces should be in-town" << std::endl;
		return false;
	}
...
bool workplace_test()
{
	...
	Workplace work(pID, xi, yi, severity_cor, psi_w, beta, type);

	if (!general_place_test(work, pID, xi, yi, severity_cor, beta, -1.0, -1.0, psi_w, {}, {}, type)){
		return false;
	}

	// Special conditions for an outside workplace
	work.set_outside_lambda(lam_out);
	if (!float_equality<double>(work.get_outside_lambda(), lam_out, 1e-5)){
		std::cerr << "Wrong outside lambda" << std::endl;
		return false;
	}
	work.adjust_outside_lambda(2.0);
	if (!float_equality<double>(work.get_outside_lambda(), lam_out_new, 1e-5)){
	...
	// And one in town
	Workplace in_town_work(pID+1, xi, yi, severity_cor, psi_w, beta, "F");
...
bool contribution_test_workplace()
{
	...
	Workplace work(pID, xi, yi, severity_cor, psi_w, beta, wtype);
...
{
	int pID = 1016; 
	double severity_cor = 1.5, beta = 0.149;
	std::string type = "carpool";

	Transit transit(pID, beta, severity_cor, type);

	if (!general_place_test(transit, pID, 0.0, 0.0, severity_cor, beta, -1.0, -1.0, -1.0, {}, {}, type)){
		return false;
	}
...
bool contribution_test_transit()
{
	...
	int n_exp = 3, n_sym = 2, n_tot = 10;
	double exp_lambda = 1.3745;

	Transit transit(pID, beta, severity_cor, type);

	if (!general_contribution_test(transit, inf_var, n_exp, n_sym, n_tot, exp_lambda)){
		return false;
	}
...
bool leisure_test()
{
	...
	double severity_cor = 1.5, beta = 0.149, lam_out = 1.34, lam_out_new = 2.68;
	std::string type = "outside";

	Leisure leisure(pID, xi, yi, severity_cor, beta, type);

	if (!general_place_test(leisure, pID, xi, yi, severity_cor, beta, -1.0, -1.0, -1.0, {}, {}, type)){
		return false;
	}
	
	// Special conditions for an outside leisure location
	leisure.set_outside_lambda(lam_out);
	if (!float_equality<double>(leisure.get_outside_lambda(), lam_out, 1e-5)){
		std::cerr << "Wrong outside lambda" << std::endl;
		return false;
	}
	leisure.adjust_outside_lambda(2.0);
	if (!float_equality<double>(leisure.get_outside_lambda(), lam_out_new, 1e-5)){
		std::cerr << "Wrong outside lambda after adjusting" << std::endl;
		return false;
	}
	...
	// And one in town
	Leisure in_town_lsr(pID+1, xi, yi, severity_cor, beta, "F");
...
bool contribution_test_leisure()
{
	...
	if (!general_contribution_test(leisure, inf_var, n_exp, n_sym, n_tot, exp_lambda)){
		return false;
	}
		...
		const std::string& type)
{
	...
	// If a workplace
	if (psi > 0.0){
		res >> test_pID >> test_x >> test_y >> test_ntot >> test_ninf >> test_ck >> test_beta >> test_psi >> test_type;
	}
	// If a hospital
