		...
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				if (agent.get_inf_variability_factor() < 0){
					std::cerr << "Agent's infectiousness variability out of range: " << std::endl;
					return false;
				}
			...
			if (agent.infected() == false){
				...
				if (agent.exposed()){
					if (!check_testing_transitions(agent, households, schools, 
							...
							carpools, public_transit, state_changes, n_tested_exposed, 
							n_tested_sy, n_waiting_for_res, n_positive, 
					...
					if (agent.tested_covid_positive()){
						if (!check_treatment_setup(agent, households, schools, hospitals, retirement_homes,
												workplaces, carpools, public_transit, n_tr_hsp, n_hsp_icu, 
												n_ih, time, dt)){
							return false;
						}
					}
			...
			if (agent.infected() == false){
				...
				if (agent.removed()){
					if (!check_symptomatic_agent_removal(agent, households, schools, hospitals, retirement_homes, 
									workplaces, carpools, public_transit, 
									state_changes, n_sy_recovering, n_sy_dying, time, dt)){
						return false;
					} 
				}
				if (!check_testing_transitions(agent, households, schools, 
							hospitals, retirement_homes, workplaces, carpools, public_transit,
...
/// Series of tests for transitions of a regular exposed agent, with initialized COVID-19
bool test_exposed_transitions_seeded()
{
	// Time in days
	double dt = 0.25;
...
/// Series of tests for transitions of a regular symptomatic agent, with initialized COVID-19
bool test_symptomatic_transitions_seeded()
{
	// Time in days
	double dt = 0.25;
	...
	for (int ti = 0; ti<=tmax; ++ti){
		...
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				if (agent.removed()){
					if (!check_symptomatic_agent_removal(agent, households, schools, hospitals, retirement_homes, 
									workplaces, carpools, public_transit, state_changes, n_sy_recovering, n_sy_dying, time, dt)){
						return false;
					} 
				}
				if (!check_testing_transitions(agent, households, schools, 
				...
				int& n_hsp, int& n_hsp_icu, int& n_ih, double time, double dt)
{
	if (agent.hospitalized()){
		...
		if (!check_hospitalized_isolation(agent, households, schools, hospitals, retirement_homes, 
								workplaces, carpools, public_transit)){
				...
				std::vector<int>& state_changes, int& n_rh, int& n_rd, double time, double dt)
{
	...
	if (state_changes.at(1)){
		...
		if (agent.retirement_home_resident()){
			...
			if (!find_in_place<Household>(households, aID, agent.get_household_ID())){
				std::cerr << "Agent recovered but not present in a household " << agent.get_recovery_time() << " " << time << std::endl;
				return false;
			}
		}
		if (agent.get_hospital_ID()>0){
